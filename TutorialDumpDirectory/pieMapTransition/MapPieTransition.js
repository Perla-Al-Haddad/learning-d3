...
function loadRichnessGraph() {
    ...
    var svg = d3.select("#WealthIndicatorMap")
        ...
        .on("mouseover", () => {
            hovering = true;
            // if (hovering) {
                d3.selectAll(".border").transition().duration(0)
                d3.select("#WealthIndicatorMap").select("svg").selectAll("path.empty").transition().duration(0)
                d3.select(".lines").selectAll("polyline").transition().duration(0);
                d3.select(".labels").selectAll("text").transition().duration(0);
                ...
                for (let i = 0; i < countries_ISO.length; i++) {
                    ...
                    let arc = svg.select("#arc_" + countries_ISO[i]);
                    path.transition().delay(0).duration(0);
                    arc.transition().delay(0).duration(0);
                }
            // }
        });
    ...
    d3.json("UN.json").then(function (e) {
        ...
        d3.json("Ranking_for_GIS_Richness.json").then(function (data) {
            ...
            svg.on("mouseout", () => {
                ...
                for (let i = 0; i < countries_ISO.length; i++) {
                    let path = svg.select("#id_" + countries_ISO[i]);
                    let bar = svg.select("#arc_" + countries_ISO[i]);
                    pie_loop(path, bar, colorScale, arc);
                }
            })
            ...
            for (let i = 0; i < countries_ISO.length; i++) {
                let cur_path = svg.select("#id_" + countries_ISO[i]);
                let cur_arc = svg.select("#arc_" + countries_ISO[i]);
                // loop(cur_path, cur_arc, arc, colorScale);
                pie_loop(cur_path, cur_arc, colorScale, arc)
            }
        })
...
function pie_loop(path, pie_arc, colorScale, arc_func) {
    ...
    path
        ...
        .attr("transform", function (d) {
            let transform_str = "";
            let cur_arc = d3.select("#WealthIndicatorMap").select("svg").select("#arc_" + d.properties.ISO3CD);
